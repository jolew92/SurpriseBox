...
class music:
    def __init__(self, songString='0 D4 8 0', looping=True, tempo=3, duty=2512, pin=None, pins=[Pin(0)]):
        ...
        self.ledRed = Pin(6, Pin.OUT)
        self.ledYellow = Pin(0, Pin.OUT) 
        self.ledGreen = Pin(11, Pin.OUT)
        self.flag = 0
    ...
    def play_until_done(self):
        self.restart()  # Reset the music state
        while self.tick():
            utime.sleep(0.04)
            pass  # Keep calling tick until the song is done
        self.stop()  # Stop the music when the song is finished